...
function EventRow({ event }) {
  ...
  const label =
    ...
    event.type === "mcp_call"  ? `[${event.data?.server}] ${event.data?.tool}` :
    event.type === "llm_call"  ? `${event.data?.model} — ${event.data?.input_tokens}+${event.data?.output_tokens} tok` :
    ...
    <div className="flex items-start gap-3 py-2 border-b border-terminal-border/40 animate-fade-in">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-terminal-dim">
          {event.agent_name} · {new Date(event.timestamp).toLocaleTimeString()}
        </div>
      </div>
...
export default function HomePage() {
  ...
  const costStr = stats.cost >= 0.01
    ...
    : stats.cost > 0 ? `$${stats.cost.toFixed(4)}` : "$0.00";
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        <StatCard label="Active Runs"  value={stats.active} color="text-terminal-green"
          sub={`${stats.active === 1 ? "1 agent" : `${stats.active} agents`} running`} />
        <StatCard label="Total Runs"   value={stats.total}  color="text-terminal-cyan" sub="all time" />
        <StatCard label="Tokens Used"  value={stats.tokens?.toLocaleString() || "0"} color="text-terminal-amber"
          sub={`${stats.llmCalls || 0} LLM calls`} />
        <StatCard label="Total Cost"   value={costStr} color="text-terminal-purple" sub="across all runs" />
      </div>
